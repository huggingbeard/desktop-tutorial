...
def generate_assistant_message(client:OpenAI, persona:str)->str:
    ...
    if persona=="Self":
        pronoun_instruction=(f"You are interviewing {name} about their own performance. "
                             "Use second-person pronouns (you/your).")
    else:
        pronoun_instruction=(f"You are interviewing the {persona_desc} about {name}. "
    ...
    system_prompt=(pronoun_instruction+"\n\n"
        ...
        "If all topics are covered, invite final reflections and summarize.")
    ...
    msgs=[{"role":"system","content":system_prompt}]+list(conversation_history())+[{"role":"system","content":status}]
...
def generate_followup_for_example(client:OpenAI,user_text:str,persona:str)->str:
    name=st.session_state.employee_name
    prompt=(f"The response about {name} was vague. "
            "Ask one short, natural follow-up question requesting a specific example or situation. "
            "Reference what was said, stay conversational.")
    r=client.chat.completions.create(model="gpt-4o-mini",
...
def generate_summary(client:OpenAI,persona:str)->str:
    ...
    r=client.chat.completions.create(model="gpt-4o-mini",
        messages=[{"role":"system","content":"Write concise per-topic interview summaries."},
...
def build_topic_csv(persona:str)->bytes:
    ...
    for t,m in TOPIC_METADATA.items():
        ...
        if not entries:
            w.writerow({"persona":persona,"topic":m["label"]}); continue
        for i,e in enumerate(entries,1):
            w.writerow({"persona":persona,"topic":m["label"],"note_index":i,
...
def main():
    ...
    if "persona" not in st.session_state or st.session_state.persona!=persona or st.session_state.get("employee_name")!=employee_name:
    ...
    if not st.session_state.finalized:
        ...
        st.download_button("Download topic notes CSV", st.session_state.csv_bytes,
                           file_name="interview_notes.csv", mime="text/csv")
        st.download_button("Download transcript (.txt)", st.session_state.txt_bytes,
