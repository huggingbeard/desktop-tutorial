"""Interactive Streamlit app for an LLM-guided interview assistant. restored midnight 12-10-25"""
...
def generate_followup_for_example(client: OpenAI, user_text: str) -> str:
    """Generate a follow-up question asking for a specific example."""
    
    prompt = (
        "The user gave a vague response in an interview. Generate ONE brief, natural follow-up question "
        "(1-2 sentences max) asking for a specific example or situation that demonstrates what they mentioned. "
    ...
    if st.session_state.finalized:
        ...
        if user_text:
            ...
            try:
                ...
                elif st.session_state.followup_count < 5 and check_if_vague(client, user_text):
                    # Ask for a concrete example
                    st.session_state.followup_count += 1
                    st.session_state.just_asked_followup = True
                    followup = generate_followup_for_example(client, user_text)
